...
public class InputTaskItemFactory : CustomCreationConverter<IInputTask>
{
    ...
    public InputTaskItemFactory(string baseDirectory, ILogger logger, IServiceCollection services,
        IPathValidatorHelper pathValidatorHelper)
    {
    ...
    {
        //objectType : AxaGuilDEv.MlCli.IInputTask
    ...
    private IInputTask Create(JObject jObject)
    {
        ...
        if (!isBaseDirectoryValid)
            _logger.LogWarning(
                "Specified base directory is invalid. Execution will be performed exclusively with paths defined in tasks.json");
        ...
        {
            case "compare":
                return Initializer.CreateTask(jObject, type, tokenEnabled, isBaseDirectoryValid, _baseDirectory,
                    _taskId, _pathValidatorHelper);
            case "parallel":
            ...
            case "wait_version_change":
                return JobVersion.Initializer.CreateTask(jObject, type, tokenEnabled, isBaseDirectoryValid,
                    _baseDirectory, _taskId, _pathValidatorHelper);
            case "copy":
                return JobCopy.Initializer.CreateTask(jObject, type, tokenEnabled, isBaseDirectoryValid, _baseDirectory,
                    _taskId, _pathValidatorHelper);
            case "callapi":
                return JobApiCall.Initializer.CreateTask(jObject, type, tokenEnabled, isBaseDirectoryValid,
                    _baseDirectory, _taskId, _services, _pathValidatorHelper);
            case "script":
                return JobScript.Initializer.CreateTask(jObject, type, tokenEnabled, isBaseDirectoryValid,
                    _baseDirectory, _taskId, _pathValidatorHelper);
            case "dataset":
                return JobDataset.Initializer.CreateTask(jObject, type, tokenEnabled, isBaseDirectoryValid,
                    _baseDirectory, _taskId, _pathValidatorHelper);
        }
