...
using AxaGuilDEv.MlCli.JobParallel;
using AxaGuilDEv.MlCli.JobScript;
using AxaGuilDEv.MlCli.JobSerial;
...
public partial class Program
{
    ...
    private async Task Run(string path, string baseDirectory, ServiceProvider provider, ServiceCollection services)
    {
        ...
        try
        {
            ...
            var inputTasks = JsonConvert.DeserializeObject<List<IInputTask>>(
                await fileLoader.ReadAllTextInFileAsync(path),
                new InputTaskItemFactory(baseDirectory, logger, services, pathValidatorHelper));
    ...
    private static async Task RunTasksAsync(ServiceProvider provider, List<IInputTask> inputTasks,
        ILogger<Program> logger)
    {
        try
        {
            foreach (var task in inputTasks) await RunTaskAsync(provider, logger, task);
        }
        catch (Exception e)
        {
