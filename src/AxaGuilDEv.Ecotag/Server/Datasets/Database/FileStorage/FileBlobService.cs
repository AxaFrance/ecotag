...
public class FileBlobService : IFileService
{
    ...
    public async Task<bool> DeleteDirectoryAsync(string blobDirectoryUri)
    {
        ...
        if (string.IsNullOrEmpty(pathEnd)) return await container.DeleteIfExistsAsync();
    ...
    public async Task<IDictionary<string, ResultWithError<FileInfoServiceDataModel, ErrorResult>>>
        GetInputDatasetFilesAsync(string blobUri,
            string datasetType)
    {
        var blobStorageName = GetBlobStorageName(blobUri);
        var containerName = GetContainerName(blobUri);
    ...
    private async Task<ResultWithError<FileInfoServiceDataModel, ErrorResult>> GetPropertiesAsync(
        ...
        int? segmentSize, int maxLevel = 2, int level = 0)
    {
        ...
        foreach (var blobhierarchyItem in blobPage.Values)
        {
            ...
            var result =
                await ListBlobsHierarchicalListing(container, blobhierarchyItem.Prefix, null, maxLevel, level + 1);
    ...
    private async Task<(string Name, ResultWithError<FileInfoServiceDataModel, ErrorResult> GetPropertiesResult)>
        GetFilePropertiesAsync(BlobItem fileBlob, string blobStorageName, string folderName, string datasetType)
    {
