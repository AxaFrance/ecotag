...
public class DatasetsRepository
{
    ...
    public static readonly IList<string> ExtentionsConvertedToPdf = ImmutableList.Create(".doc", ".docx", ".xls",
        ".xlsx", ".ppt", ".pptx", ".tif", ".tiff", ".rtf", ".odt", ".ods", ".odp");
    ...
    public DatasetsRepository(DatasetContext datasetsContext, IFileService fileService, IMemoryCache cache,
        ImportDatasetFilesService importDatasetFilesService)
    {
    ...
    public async Task<ResultWithError<string, ErrorResult>> CreateDatasetAsync(CreateDataset createDataset)
    {
        ...
        try
        {
            ...
            if (createDataset.ImportedDatasetName != null)
                // Fire and Forget
                _importDatasetFilesService.ImportFilesAsync(createDataset.ImportedDatasetName,
                    datasetModel.Id.ToString(), datasetModel.Type, datasetModel.CreatorNameIdentifier);
        }
        catch (Exception)
        {
    ...
    public async Task<IList<ListDataset>> ListDatasetAsync(IList<DatasetLockedEnumeration> locked,
        IList<string> groupIds = null)
    {
        ...
        if (locked is { Count: > 0 })
        {
            if (groupIds != null)
                datasets = await _datasetContext.Datasets.AsNoTracking()
                    .Where(dataset => locked.Contains(dataset.Locked) && groupIds.Contains(dataset.GroupId.ToString()))
        ...
        else
        {
            if (groupIds != null)
                datasets = await _datasetContext.Datasets.AsNoTracking()
                    .Where(dataset => groupIds.Contains(dataset.GroupId.ToString()))
                    .OrderByDescending(d => d.CreateDate).Include(dataset => dataset.Files)
                    .ToListAsync();
            else
                datasets = await _datasetContext.Datasets.AsNoTracking().OrderByDescending(d => d.CreateDate)
    ...
    public async Task<bool> LockAsync(string datasetId)
    {
        ...
        {
            var filenames = await _datasetContext.Files.Where(f => f.DatasetId == Guid.Parse(datasetId))
            ...
            if (isContainFileToConvertPdf) locked = DatasetLockedEnumeration.LockedAndWorkInProgress;
        }
    ...
    public async Task<ResultWithError<FileServiceDataModel, ErrorResult>> GetFileAsync(string datasetId, string fileId,
        bool isRetrievePdfIfAvailable = false)
    {
        ...
        var file = await _datasetContext.Files.Include(f => f.Dataset).AsNoTracking().Where(file =>
                file.Id == new Guid(fileId) && file.DatasetId == new Guid(datasetId)).Select(f =>
                new { Filename = f.Name, f.ContentType, BlobDirectoryUri = f.Dataset.BlobUri, f.Dataset.Type })
            .FirstOrDefaultAsync();
        if (file == null)
        {
        ...
        if (file.Type == DatasetTypeEnumeration.Document &&
            ExtentionsConvertedToPdf.Contains(Path.GetExtension(file.Filename)) && isRetrievePdfIfAvailable)
        {
            ...
            if (isFileExist)
            {
                ...
                if (!fileDownloadedPdf.IsSuccess) return fileDownloadedPdf;
        ...
        if (!fileDownloaded.IsSuccess) return fileDownloaded;
