...
public class AnnotationsRepository
{
    ...
    public AnnotationsRepository(DatasetContext datasetsContext, IServiceScopeFactory serviceScopeFactory,
        IMemoryCache cache)
    {
    ...
    public async Task<AnnotationStatus> AnnotationStatusAsync(string projectId, string datasetId,
        int numberAnnotation = 1)
    {
        ...
        var percentageNumberAnnotationsDone =
            Convert.ToDouble(numberAnnotationsDone) / Convert.ToDouble(numberAnnotationsToDo) * 100;
        if (percentageNumberAnnotationsDone >= 100) percentageNumberAnnotationsDone = 99;
    ...
    public async Task<IList<ReserveOutput>> ReserveAsync(string projectId, string datasetId,
        string creatorNameIdentifier, string fileId = null, int numberAnnotation = 1, int numberToReserve = 10)
    {
        var query =
            _datasetsContext.Files.Where(f => f.DatasetId == new Guid(datasetId)
                                              && f.Annotations.Count(a => a.ProjectId == new Guid(projectId)) <
                                              numberAnnotation
                                              && f.Annotations.Count(a =>
                                                  a.ProjectId == new Guid(projectId) &&
                                                  a.CreatorNameIdentifier == creatorNameIdentifier) == 0
            ).Select(file =>
                new
                {
                    FileId = file.Id,
        ...
        {
            var querySingle =
                _datasetsContext.Files.Where(f => f.DatasetId == new Guid(datasetId) && f.Id == new Guid(fileId))
                    .Select(file =>
                        new
                        {
                            FileId = file.Id,
                            FileName = file.Name,
                            Reservation = file.Reservations.SingleOrDefault(r => r.ProjectId == new Guid(projectId)),
                            Annotation = file.Annotations.Where(a =>
                                    a.ProjectId == new Guid(projectId) &&
                                    a.CreatorNameIdentifier == creatorNameIdentifier)
        ...
        foreach (var result in results)
        {
            ...
            if (reservation == null)
                _datasetsContext.Reservations.Add(new ReservationModel
                    { FileId = result.FileId, TimeStamp = ticks, ProjectId = new Guid(projectId) });
            else
    ...
    public async Task<ResultWithError<string, ErrorResult>> CreateOrUpdateAnnotation(
        SaveAnnotationInput saveAnnotationInput)
    {
        ...
        {
            annotation =
                await _datasetsContext.Annotations.FirstOrDefaultAsync(a =>
                    a.Id == new Guid(saveAnnotationInput.AnnotationId));
            if (annotation == null)
            {
