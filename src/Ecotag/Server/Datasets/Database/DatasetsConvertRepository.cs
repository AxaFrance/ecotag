...
public class DatasetsConvertRepository
{
    ...
    public DatasetsConvertRepository(DatasetContext datasetsContext, IFileService fileService,
        DocumentConverterToPdf documentConverterToPdf, IServiceScopeFactory serviceScopeFactory)
    {
    ...
    public async Task ConvertFileToPdfAsync(string datasetId)
    {
        ...
        if (dataset.Type != DatasetTypeEnumeration.Document) return;
        ...
        foreach (var chunk in chunks) promises.Add(ConvertToPdfAsync(_serviceScopeFactory, chunk.ToList(), dataset.Id));
    ...
    private static async Task ConvertToPdfAsync(IServiceScopeFactory serviceScopeFactory,
        IList<FileToConvertPdf> filenames, Guid datasetId)
    {
        ...
        while (count > 0)
        {
            ...
            if (reservations.Count > 0) await datasetContext.SaveChangesAsync();
        }
    }

    private record FileToConvertPdf
    {
