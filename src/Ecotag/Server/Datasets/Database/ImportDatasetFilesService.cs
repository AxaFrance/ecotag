...
public class ImportDatasetFilesService
{
    ...
    public async Task ImportFilesAsync(string importedDatasetName, string datasetId, DatasetTypeEnumeration datasetType,
        string creatorNameIdentifier)
    {
        await Task.Run(async () =>
        {
            ...
            var logger =
                (ILogger<ImportDatasetFilesService>)serviceProvider.GetService(
                    typeof(ILogger<ImportDatasetFilesService>));
            try
            {
                await ImportDatasetFilesAsync(fileService, datasetContext, _cache, importedDatasetName, datasetId,
                    datasetType, creatorNameIdentifier);
            }
    ...
    private static async Task ImportDatasetFilesAsync(IFileService fileService, DatasetContext datasetContext,
        IMemoryCache cache, string importedDatasetName, string datasetId, DatasetTypeEnumeration datasetType,
        string creatorNameIdentifier)
    {
        ...
        var successFiles = filesResult
            .Where(x => x.Value.IsSuccess).Where(f =>
                FileValidator.IsFileExtensionValid(f.Key, datasetType.ToString()) &&
                FileValidator.IsFileSizeValid(f.Value.Data.Length)).ToList();
