...
{
    public CompareToken(string fileName, MlCli.Program.HttpResult left, MlCli.Program.HttpResult right)
    {
        FileName = fileName;
        Left = left;
        Right = right;
    ...
    public MlCli.Program.HttpResult Left { get; set; }
    public MlCli.Program.HttpResult Right { get; set; }
}
...
{
    public ItemInfo(string fileName, MlCli.Program.HttpResult left, MlCli.Program.HttpResult right)
    {
        FileName = fileName;
        Left = left;
        Right = right;
...
public class ComparesController : ControllerBase
{
    ...
    private static EditorContent ReformatEditorContent(EditorContent data)
    {
        ...
        if (contentBodyLeft != null) content.Left.Body = JsonConvert.SerializeObject(contentBodyLeft);

        dynamic contentBodyRight = JsonConvert.DeserializeObject(content.Right.Body);
        if (contentBodyRight != null) content.Right.Body = JsonConvert.SerializeObject(contentBodyRight);
