...
namespace Ml.Cli.JobApiCall
{
    public class ApiCallFiles
    {
        ...
        public ApiCallFiles(IHttpClientFactory httpClientFactory, IFileLoader fileLoader, IFileHandler fileHandler, ILogger logger)
        {
        ...
        private void FindElementsTokens(JToken containerToken, List<FileUrls> urlsList, Dictionary<string, JsonsList> jsonsDict)
        {
        ...
        public async Task ApiCallFilesAsync(string fileName, string json, Callapi inputTask)
        {
            ...
            try
            {
                ...
                foreach (var regex in tempStringsArray)
                {
        ...
        private async Task DownloadFilesAsync(HttpClient httpClient, string fileName, List<FileUrls> urlsList, IDictionary<string, JsonsList> jsonsDict, string[] stringsArray, Callapi inputTask)
        {
        ...
        {
            if (stringsArray == null || input == null)
            {
                return false;
            }
