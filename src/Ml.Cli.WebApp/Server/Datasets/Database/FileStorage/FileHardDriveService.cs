...
public class FileHardDriveService : IFileService
{
    ...
    public async Task<ResultWithError<FileServiceDataModel, ErrorResult>> DownloadAsync(string blobFileUri)
    {
        ...
        string contentType;
        if(!provider.TryGetContentType(blobFileUri, out contentType))
        {
    ...
    public Task<IDictionary<string, ResultWithError<FileInfoServiceDataModel, ErrorResult>>> GetInputDatasetFilesAsync(string blobUri, string datasetType)
    {
        throw new System.NotImplementedException();
    }
