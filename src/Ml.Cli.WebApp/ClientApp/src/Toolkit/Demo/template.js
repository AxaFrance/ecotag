...
export const zoneAsync = (cv) => async (sceneUrl, imgDescription, goodMatchSizeThreshold = 6) => {
    ...
    const {image: imgResized, ratio} = imageResize(cv)(imgCv, 1600);
   ...
   // const youhou = detectAndComputeSerializable(cv)( imgVersoCvTemplateResized);
    ...
    if(!result){
        return {expectedOutput:[], url: base64Url, confidenceRate :0, isGray, croppedContoursBase64:null, outputInfo:null };
    }
    ...
    let outputInfo = {
        result : croppedContours.map(cc => {return {
            homography: {
                from: cc.from,
                to: cc.to
            },
