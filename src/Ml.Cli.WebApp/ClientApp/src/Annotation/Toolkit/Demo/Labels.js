    ...
    <div className="tagovertext-labels__group-container">
      {group.map(label => (
        <div key={label.id} className="tagovertext-labels__input-container">
          <CustomInput currentLabelId={currentLabelId} label={label} selectLabel={selectLabel} onChange={onChange} />
          <div className={`tagovertext-labels__input-conf ${getConfModifier(label.conf)}`} title={label.conf} />
        </div>
      ))}
    </div>
  );
