...
public class CompareUnitTest
{
    ...
    public async Task CompareShouldCallCompareAsync()
    {
        ...
        fileLoader.Setup(mock =>
                mock.ReadAllTextInFileAsync(
                    PathAdapter.AdaptPathForCurrentOs("baseDirectory/ml/raw_ap/input/left/{FileName}.pdf.json")))
            .Returns(Task.FromResult(jsonContent));
        fileLoader.Setup(mock =>
                mock.ReadAllTextInFileAsync(
                    PathAdapter.AdaptPathForCurrentOs("baseDirectory/ml/raw_ap/input/right/{FileName}.pdf.json")))
            .Returns(Task.FromResult(jsonContent));
        ...
        fileLoader.Verify(
            mock => mock.WriteAllTextInFileAsync(Path.Combine(inputTask.OutputDirectory, "{FileName}.pdf.json"),
    ...
    public async Task ShouldSaveNonHttpResultFiles()
    {
        ...
        var compareResult = new CompareResult
            { FileName = "{FileName}.json", Left = httpResultLeft, Right = httpResultRight };
        ...
        var fileLoader = new Mock<IFileLoader>();
        fileLoader.Setup(mock => mock.EnumerateFiles(It.IsAny<string>(), It.IsAny<string>()))
            .Returns(new List<string> { "{FileName}.json" });
        fileLoader.Setup(mock =>
                mock.ReadAllTextInFileAsync(
                    PathAdapter.AdaptPathForCurrentOs("baseDirectory/ml/raw_ap/input/left/{FileName}.json")))
            .Returns(Task.FromResult(contentLeft));
        fileLoader.Setup(mock =>
                mock.ReadAllTextInFileAsync(
                    PathAdapter.AdaptPathForCurrentOs("baseDirectory/ml/raw_ap/input/right/{FileName}.json")))
            .Returns(Task.FromResult(contentRight));
        fileLoader.Setup(mock => mock.FileExists(It.IsAny<string>())).Returns(true);
        fileLoader.Setup(mock => mock.CreateDirectory(It.IsAny<string>()));
        fileLoader.Setup(mock => mock.WriteAllTextInFileAsync(It.IsAny<string>(), It.IsAny<string>()));
        ...
        fileLoader.Verify(mock =>
            mock.WriteAllTextInFileAsync(
                PathAdapter.AdaptPathForCurrentOs("baseDirectory/ml/raw_ap/output/{FileName}.json"), expectedString));
    }
    ...
    {
        var jsonContent =
            "{\"type\": \"compare\",\"enabled\": true,\"onFileNotFound\": \"warning\",\"leftDirectory\": \"licenses/groundtruth/jsons\",\"rightDirectory\": \"licenses/output/{start-date}/jsons\",\"outputDirectory\": \"licenses/compares\",\"fileName\": \"compare-licenses-{start-date}.json\"}";
        var jObject = JObject.Parse(jsonContent);
        var pathValidatorHelper = new Mock<IPathValidatorHelper>();

        var compareResult = Initializer.CreateTask(jObject, "compare", false, true, "baseDirectory", "1",
