...
public class ScriptUnitTest
{
    ...
    public async Task ShouldExecuteScript()
    {
        ...
        fileLoader.Setup(mock => mock.EnumerateFiles(It.IsAny<string>(), It.IsAny<string>()))
            .Returns(new List<string> { "toto_pdf.json" });
        ...
        fileLoader.Verify(
            mock => mock.WriteAllTextInFileAsync(
                PathAdapter.AdaptPathForCurrentOs(Path.Combine(inputTask.OutputDirectory, "toto_pdf.json")),
    ...
    {
        var jsonContent =
            "{\"type\": \"script\",\"id\": \"script_task\",\"enabled\": true,\"fileDirectory\": \"licenses/output\",\"outputDirectory\": \"licenses/scripts\",\"script\": \"try{ console.WriteLine('Write your JS code on this attribute (not necessarily in a try-catch block).') } catch(ex){ console.WriteLine(ex.toString) }\"}";
